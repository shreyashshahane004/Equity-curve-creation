...
async function run() {
  ...
  let count = 0;
  console.log("Details for months where the first 4 trades are losses (Strategy 1):");
  ...
  for (const [monthKey, trades] of Object.entries(months)) {
    ...
    if (trades.length > 3) {
      ...
      const isFourthLoss = fourthTrade.r_value < 0;

      if (isFirstLoss && isSecondLoss && isThirdLoss && isFourthLoss) {
        count++;
        let maxR = 0;
        let cumulativeR = 0;
