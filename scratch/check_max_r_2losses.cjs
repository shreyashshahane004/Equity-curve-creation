...
async function run() {
  ...
  console.log("Months where the first 2 trades are losses (Strategy 1):");
  ...
  for (const [monthKey, trades] of Object.entries(months)) {
    ...
    if (trades.length > 1) {
      ...
      if (isFirstLoss && isSecondLoss) {
        ...
        console.log(`- ${monthName} ${yearValue}: Max reached R = ${parseFloat(maxR.toFixed(2))}R`);
      }
    }
  }
}
