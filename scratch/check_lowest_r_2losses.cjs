...
async function run() {
  ...
  console.log("Lowest R for months where the first 2 trades are losses (Strategy 1):");
  ...
  for (const [monthKey, trades] of Object.entries(months)) {
    ...
    if (trades.length > 1) {
      ...
      if (isFirstLoss && isSecondLoss) {
        let lowestR = 0;
        let cumulativeR = 0;
        
        for (const trade of trades) {
          cumulativeR += trade.r_value;
          if (cumulativeR < lowestR) {
            lowestR = cumulativeR;
          }
        }
        ...
        console.log(`- ${monthName} ${yearValue}: Lowest reached R = ${parseFloat(lowestR.toFixed(2))}R`);
      }
    }
  }
}
